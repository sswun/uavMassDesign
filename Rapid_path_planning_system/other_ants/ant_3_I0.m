%% 蚁群算法中改进蚂蚁类设计3.0_I0
% 适用包括区域覆盖搜索的各类TSP问题,softfunchange,群类多样化
classdef ant_3_I0
    properties
        position; % 当前蚂蚁位置
        way; % 蚂蚁走过的路径
        ...
        length; % 路径长度
        length_t; % 当前走过距离
        numchoose; % 运输者选择最近的numchoose个位置作为备选移动位置
    end
    methods
        % 初始化
        function obj = ant_3_I0(pos,numofpositions,numchoose)
            obj.position=pos;
        ...
        function obj=choose_next_position_trans(obj,w_adj,tao_pheromonee,...
                tao_pheromonet,alpha_t,beta_t,alpha_e,beta_e)
            ...
            for i=1:obj.numofpositions
                if(~ismember(i,obj.way))
                    numtocho=[numtocho,i];
                    probility_nextnode(i)=abs(tao_pheromonet(obj.position,i)^alpha_t*...
                        (1/w_adj(obj.position,i))^beta_t);
                end
            end
            % 选择探索者确定的较好的numchoose个位置作为待选节点
            if(size(numtocho,2)>obj.numchoose)
                for i=1:obj.numchoose
                    for j=size(numtocho,2)-1:-1:1
                        if(abs(tao_pheromonee(obj.position,numtocho(j))^alpha_e*...
                        (1/w_adj(obj.position,numtocho(j)))^beta_e)<...
                        abs(tao_pheromonee(obj.position,numtocho(j+1))^alpha_e*...
                        (1/w_adj(obj.position,numtocho(j+1)))^beta_e))
