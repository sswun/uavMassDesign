function [uav_1,agent] = Rapid_PP_TPP_IDDPG_try(uav_0,target,num_prelearn,numtofly,field_tree,trees)
...
for i=1:num_prelearn
    ...
    while ~done
        ...
        next_position = findProperPoint_Tofly_try(uav_0.position(end,:),direction,d_dist,...
            field_tree,trees);
...
if check == 0
    if norm(uav_2.position(end,:) - target) < uav_1.speed(end,:)*uav_1.dt
        for i=2:size(uav_2.position,1)
            uav_1 = uav_1.FlyToNextPoint_flexible_try(uav_2.position(i,:),[],[],trees,field_tree);
        end
        return
    end
end
if norm(uav_0.position(end,:) - target) < uav_1.speed(end,:)*uav_1.dt
    for i=2:size(uav_0.position,1)
        uav_1 = uav_1.FlyToNextPoint_flexible_try(uav_0.position(i,:),[],[],trees,field_tree);
    end
    return
end
uav_1 = uav_1.FlyToNextPoint_flexible_try(target,[],[],trees,field_tree);
return
end
